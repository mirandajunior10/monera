...
export default function OverlayReceita(props) {
  ...
  return (
    <Overlay
      visible={context.state.receitaOverlay}
      closeOnTouchOutside
      onBackdropPress={() => { handleCancel(context) }}
      onBackButtonPress={() => { handleCancel(context) }}
      onDismiss={() => { handleCancel(context) }}
      ...
      <View style={styles.buttonContainer}>
        ...
        <Button
          ...
          titleStyle={[styles.buttonTitle, styles.saldoPositivo]}
          disabledTitleStyle={styles.buttonTitle}
          onPress={() => { handleAddTransaction(context, 1) }}
        />
      </View>
    </Overlay>
  )
